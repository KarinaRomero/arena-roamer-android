...
public class Player {
	...
	public void moveForward(float delta, Thread t) {
		velocity.set(-(float)(Math.sin(Math.toRadians(getRotation())) * speed), (float)(Math.cos(Math.toRadians(getRotation())) * speed));
		rotationModifier = 0;
		tryMove(delta,t);
	}
	
	public void moveBackward(float delta, Thread t) {
		velocity.set((float)(Math.sin(Math.toRadians(getRotation())) * speed), -(float)(Math.cos(Math.toRadians(getRotation())) * speed));
		rotationModifier = 0;
		tryMove(delta,t);
	}

	public void strafeLeft(float delta, Thread t) {
		velocity.set(-(float)(Math.cos(Math.toRadians(getRotation())) * speed), -(float)(Math.sin(Math.toRadians(getRotation())) * speed));
		rotationModifier = 0;
		tryMove(delta,t);
	}
	
	public void strafeRight(float delta, Thread t) {
		velocity.set((float)(Math.cos(Math.toRadians(getRotation())) * speed), (float)(Math.sin(Math.toRadians(getRotation())) * speed));
		rotationModifier = 0;
		tryMove(delta,t);
	}

	public void turnLeft(float delta, Thread t) {
		velocity.set(0.0f, 0.0f);
		rotationModifier = speed * 50 * delta;
		tryMove(delta,t);
	}
	
	public void turnRight(float delta, Thread t) {
		velocity.set(0.0f, 0.0f);
		rotationModifier = speed * -50 * delta;
		tryMove(delta,t);
	}
	...
	private void tryMove(float delta, Thread t) {
