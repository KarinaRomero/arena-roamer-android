	...
	private void handleInput(float delta) {
		...
		if (Gdx.app.getType() == ApplicationType.Android) {
			if (Gdx.input.getAccelerometerY() < -2) {
				c= new Cliente("192.168.1.76",5000,delta,Input.Keys.Z);
				t= new Thread(c);
				player.strafeLeft(delta,t);
			}
			if (Gdx.input.getAccelerometerY() > 2) {
				c= new Cliente("192.168.1.76",5000,delta,Input.Keys.X);
				t= new Thread(c);
				player.strafeRight(delta,t);

			}
			if (Gdx.input.getAccelerometerX() < 7) {
				c= new Cliente("192.168.1.76",5000,delta,Input.Keys.UP);
				t= new Thread(c);
				player.moveForward(delta,t);
			}
			if (Gdx.input.getAccelerometerX() > 9) {
				c= new Cliente("192.168.1.76",5000,delta,Input.Keys.DOWN);
				t= new Thread(c);
				player.moveBackward(delta,t);
			}
			
			if (Gdx.input.isTouched()) {
				if (Gdx.input.getX() < ArenaRoamer.WIDTH / 2) {
					c= new Cliente("192.168.1.76",5000,delta,Input.Keys.LEFT);
					t= new Thread(c);
					player.turnLeft(delta,t);
				}
				if (Gdx.input.getX() > ArenaRoamer.WIDTH / 2) {
					c= new Cliente("192.168.1.76",5000,delta,Input.Keys.RIGHT);
					t= new Thread(c);
					player.turnRight(delta,t);
				}
			}
